...
export default async function initializeAmmAccount(data: AmmAccountData) {
  ...
  try {
    ...
    const simulated = await rpc.simulateTransaction(encoder64, {
      encoding: "base64",
      commitment: "confirmed"
    }).send();
...
async function AmmaccountsCreator(data: AmmAccountData): Promise<(AccountMeta | AccountSignerMeta)[]> {
  ...
  const admin = await createKeyPairSignerFromBytes(new Uint8Array([10, 92, 27, 184, 114, 49, 64, 68, 52, 212, 240, 162, 153, 140, 141, 89, 87, 248, 49, 41, 56, 143, 73, 183, 89, 195, 22, 183, 89, 59, 194, 35, 227, 129, 76, 82, 217, 163, 19, 224, 13, 216, 217, 51, 54, 111, 34, 248, 164, 178, 79, 111, 66, 116, 188, 189, 132, 117, 72, 191, 89, 77, 133, 134]))
  accounts.push({
 ...
 //  amm_token_account (writable)
  const [amm_token_account_address] = await getProgramDerivedAddress({
    programAddress: address(process.env.AMM_PROGRAM_ADDRESS!),
    seeds: [
  ...
  const [lp_token_mint_account_address] = await getProgramDerivedAddress({
    programAddress: address(process.env.AMM_PROGRAM_ADDRESS!),
    seeds: ['mint', addressEncoder.encode(address(amm_token_account_address)), addressEncoder.encode(address(data.token_a_mint_account)), addressEncoder.encode(address(data.token_b_mint_account))],
  })
  ...
  const [admin_lp_token_account_address] = await getProgramDerivedAddress({
    programAddress: address(process.env.AMM_PROGRAM_ADDRESS!),
    seeds: [
      addressEncoder.encode(address(data.admin_account)), 
